...
impl RawMaster {
    ...
    fn append_magic_template(
        items_to_build: &mut Vec<NewMagicItem>,
        item: &super::item_structs::Item,
    ) {
    ...
    fn build_base_magic_item(&self, nmw: &NewMagicItem) -> super::item_structs::Item {
        ...
        base_item_copy.magic = Some(super::item_structs::MagicItem {
            ...
            naming: base_item_copy
                ...
                .unidentified_name
                .clone(),
            cursed: if nmw.bonus < 0 { Some(true) } else { None },
        });
        ...
        if let Some(mut weapon) = base_item_copy.weapon.as_mut() {
            ...
            if let Ok(die) = parse_dice_string(&weapon.base_damage) {
                let final_bonus = die.bonus + nmw.bonus;
                if final_bonus > 0 {
                    weapon.base_damage = format!("{}d{}+{}", die.n_dice, die.die_type, final_bonus);
                } else if final_bonus < 0 {
                    weapon.base_damage = format!("{}d{}-{}", die.n_dice, die.die_type, final_bonus);
                } else {
                    weapon.base_damage = format!("{}d{}", die.n_dice, die.die_type);
                }
            }
        }
    ...
    fn build_magic_weapon_or_armor(&mut self, items_to_build: &[NewMagicItem]) {
        for nmw in items_to_build.iter() {
            let new_item = self.build_base_magic_item(nmw);
            let real_name = new_item.name.clone();
            self.raws.items.push(new_item);
            self.item_index
